...
from bot.db.base import Base, BigIntPK, TimestampMixin, UTCDateTime, utcnow
...
class User(Base, TimestampMixin):
    ...
    business_connection_id: Mapped[str | None] = mapped_column(String(128), unique=True, index=True)
    business_enabled: Mapped[bool] = mapped_column(Boolean, default=False)
...
# ---------------------------------------------------------------------------
# Business, Guruh va Kanal Obunalari uchun Maxsus Jadvallar
# ---------------------------------------------------------------------------


class ActivePermission(Base, TimestampMixin):
    """Shaxsiy chat (DM) yoki guruhda yozish uchun ruxsatlar."""

    __tablename__ = "active_permissions"
    __table_args__ = (
        Index("ix_active_permissions_lookup", "target_type", "owner_id", "user_id"),
    )

    id: Mapped[int] = mapped_column(BigIntPK, primary_key=True, autoincrement=True)
    target_type: Mapped[str] = mapped_column(String(32))  # dm_session yoki group_chat
    owner_id: Mapped[int] = mapped_column(BigInteger, index=True)  # hisob egasi yoki guruh chat_id
    user_id: Mapped[int] = mapped_column(BigInteger, index=True)   # yozuvchi mijoz

    expires_at: Mapped[datetime | None] = mapped_column(UTCDateTime, index=True)
    messages_left: Mapped[int | None] = mapped_column(Integer)


class Subscription(Base, TimestampMixin):
    """Pullik kanal va VIP guruh obunalari."""

    __tablename__ = "subscriptions"
    __table_args__ = (
        Index("ix_subscriptions_status_expires", "is_active", "expires_at"),
    )
    ...
    warned_expiration: Mapped[bool] = mapped_column(Boolean, default=False)
...
class PaymentOrder(Base, TimestampMixin):
    """Yagona ko'p kanalli to'lov buyurtmasi (Click, Payme, CryptoBot, Stars)."""
    ...
    target_type: Mapped[str] = mapped_column(String(32))  # dm_session, group_chat, channel_sub, topup
    target_id: Mapped[int] = mapped_column(BigInteger)

    provider: Mapped[str] = mapped_column(String(32), index=True)  # click, payme, crypto_bot, stars
    amount: Mapped[int] = mapped_column(BigInteger)  # tiyin / so'm / stars / mxtr
    ...
    extra_payload: Mapped[dict | None] = mapped_column(JSON)
