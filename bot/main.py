"""Bot va FastAPI Web Serverni ishga tushirish."""
...
async def set_commands(bot: Bot) -> None:
    ...
    await bot.set_my_commands(
        [
            BotCommand(command=name, description=get(key, settings.default_language))
            for name, key in private_commands
        ],
    ...
    await bot.set_my_commands(
        [
            BotCommand(command=name, description=get(key, settings.default_language))
            for name, key in group_commands
        ],
...
async def set_profile(bot: Bot) -> None:
    """Bot profili: tavsif va qisqa tavsif."""
    await bot.set_my_description(get("botprofile.description", settings.default_language))
    await bot.set_my_short_description(get("botprofile.short_description", settings.default_language))
    for lang in SUPPORTED_LANGUAGES:
        await bot.set_my_description(get("botprofile.description", lang), language_code=lang)
        await bot.set_my_short_description(get("botprofile.short_description", lang), language_code=lang)
...
async def on_startup(bot: Bot) -> None:
    ...
    try:
        ...
        logger.exception("Bot profilini o'rnatib bo'lmadi")
...
async def main() -> None:
    ...
    # FastAPI Web Server (Mini App + Click/Payme/Crypto Webhooks)
    fastapi_app = create_app(bot, dp)
    ...
    logger.info(
        "🌐 Web server va Mini App ishga tushdi: http://%s:%s/app",
    ...
    try:
        if settings.use_webhook:
            # Webhook rejimida faqat web serverni ishlatamiz
            await server.serve()
        else:
            # Polling rejimida bot polling va FastAPI server bir vaqtda ishlaydi
